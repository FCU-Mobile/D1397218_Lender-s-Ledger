...
// In Xcode, you would put this in its own file: `DeletedView.swift`
            ...
            .alert("Confirm Deletion", isPresented: $showingDeleteConfirmation) {
                ...
                Button("Delete", role: .destructive) {
                    if let item = itemToDelete,
                       let index = viewModel.deletedItems.firstIndex(where: { $0.id == item.id }) {
                        viewModel.permanentlyDeleteItem(at: IndexSet(integer: index))
                    }
                    itemToDelete = nil
                }
            } message: {
